...
```{r}
#first library a few packages that may be used in this analysis
library(highcharter)
library(tidyverse)
library(downloader)
library(rgdal)
...
Now, we need to collect data from different websites, I have already done this step and saved them in my computer
...
```{r}
#house_price data contains median, mean, and sales of house_price, I choose #mean figure to do next analysis
...
#then summary this data to see what it likes

summary(house_price_new)
```

```{r}
#then the data of loneliness
...
summary(old_loneliness_new)
```

```{r}
#then peoperty period data
...
#we choose property which built after 2000 as new building to see how new buildings influence loneliness of old people because they may prefer old buildings which can provide them with familiar environment
...
summary(property_period_new)
```

```{r}
#next is the data of rent percentage
...
#we combine the figures of public rent and private rent to get total rent percentage
...
summary(rent_percentage_new)
```

```{r}
#finally is the data of bad health
...
summary(bad_health_new)
```

```{r}
#now we can remove the intermediate data
...
```{r}
summary(loneliness_geo)
```

```{r}
#let's plot it to see the distribution of loneliness
...
```
```{r}
qtm(loneliness_reg, fill = "model_final_res")
```
